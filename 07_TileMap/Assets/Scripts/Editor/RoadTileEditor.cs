...
[CustomEditor(typeof(RoadTile), true)]
public class RoadTileEditor : Editor
{
    ...
    /// <summary>
    /// 인스팩터창에서 필요 정보를 그리는 함수
    /// </summary>
    public override void OnInspectorGUI()
    {
        base.OnInspectorGUI();      // 기본적으로 그리던 것은계속 그림

        if (roadTile != null && roadTile.sprites != null)           // roadTile과 스프라이트가 있으면
        {
            Texture2D texture;
            ...
            {
                texture = AssetPreview.GetAssetPreview(sprite);                     // 스프라이트를 텍스쳐로 바구고
                GUILayout.Label("", GUILayout.Height(64), GUILayout.Width(64));     // 크기 잡고
                GUI.DrawTexture(GUILayoutUtility.GetLastRect(), texture);           // 크기 잡은 곳에서 텍스쳐 그리기
            }
