...
public class LoadingScene : MonoBehaviour
{
    ...
    private void Awake()
    {
        inputActions = new PlayerInputAction();         // 인풋 액션 만들기
    }
    ...
    private void OnDisable()
    {
        ...
        inputActions.UI.Disable();                      // 필요한 액션 맵 비활성화
    }
    ...
    private void Start()
    {
        ...
        loadingTextCoroutine = LoadingTextProgress();           // 코루틴 저장해 놓기(나중에 stop을 시켜야 하기 때문에 저장)
    ...
    /// <summary>
    /// 키보드가 아무거나 눌려지거나 마우스 좌/우 버튼이 눌러졌을 때 실행
    /// </summary>
    /// <param name="_"></param>
    private void Press(InputAction.CallbackContext _)
    {
        ...
        {
            async.allowSceneActivation = true;      // 로드한 씬을 활성화 하도록 설정
        }
    }
    ...
    IEnumerator LoadScene()
    {
        ...
        loadingComplete = true;                                 // 로딩 끝났다고 표시

        loadingText.text = "Loading\nComplete.";                // 로딩이 끝났다고 텍스트로 표시
    }
    ...
    {
        WaitForSeconds wait = new WaitForSeconds(0.2f);     // 가비지가 계속 생성되는 것을 방지하기 위해 미리 만들기 + 글자가 변경되는 간격
        int point = 0;                                      // texts에서 몇 번째를 보여줄 것인지 결정
        ...
        while (true)
        {
            ...
            //    loadingText.text = "Loading";
            //}
            loadingText.text = texts[point];                // loadingText 변경

            point++;                                        // index 증가
            point %= texts.Length;                          // 배열 크기 안쪽만 값이 설정 되도록 처리

            yield return wait;                              // waitTime초 만큼 대기
        }
    }
}
