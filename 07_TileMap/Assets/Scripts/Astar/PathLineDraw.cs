...
public class PathLineDraw : MonoBehaviour
{
    ...
    public void DrawPath(GridMap map, List<Vector2Int> path)
    {
        ...
        {
            Vector2 wolrdPos = map.GridToWorld(node);           // 월드 좌표를 계산
            ...
            // 라인 랜더러에 설정 (라인 랜더러의 Position은 local 좌표로 설정되어야 하기 때문에 변환과정 추가
            index++;                                            // 라인 랜더러의 Pisition의 index 변경
        }
    }
    ...
    {
        lineRenderer.positionCount = 0;         // 라인 랜더러가 가진 위치 제거
